...
const config = window.baseUserConfig || {};

// Mobile-specific improvements
document.addEventListener('DOMContentLoaded', function() {
    // Add mobile class to body for specific targeting
          ...
          if (e.target.matches('input, select, textarea')) {
            if (event === 'focus') {
              ...
              setTimeout(() => {
                const el = document.querySelector('meta[name=viewport]');
                if (el) {
                  el.setAttribute('content', el.getAttribute('content').replace(/maximum-scale=[0-9\.]+/g, 'maximum-scale=5'));
                }
              }, 500);
            }
          }
    ...
    document.addEventListener('touchmove', function(e) {
      ...
      if ((window.pageYOffset === 0 && touchDiff > 0) || 
          (window.pageYOffset >= document.body.scrollHeight - window.innerHeight && touchDiff < 0)) {
        e.preventDefault();
      }
    }, { passive: false });
  });
