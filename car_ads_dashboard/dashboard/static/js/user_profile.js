...
const config = window.userProfileConfig;
...
document.querySelectorAll('.password-toggle').forEach(toggle => {
    ...
    form.addEventListener('submit', function(e) {
      ...
      if (!isValid) {
        ...
        if (this.id === 'profileForm') {
          ...
          const newUsername = document.getElementById(config.profileUsernameFieldId).value;
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    if (profileSuccessFound && sessionStorage.getItem('usernameChanged') === 'true') {
      ...
      if (newUsername) {
        ...
        setTimeout(() => {
          window.location.href = `/dashboard/user/${newUsername}/profile/`;
        }, 2000);
      }
    }
  ...
  // Initialize error states on page load
  document.addEventListener('DOMContentLoaded', function() {
    // Add has-error class to password wrappers that have errors
